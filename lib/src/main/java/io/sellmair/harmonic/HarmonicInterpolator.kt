...
class HarmonicInterpolator private constructor(private val omega: Double, private val gamma: Double) : Interpolator {
    ...
    override fun getInterpolation(p0: Float): Float {
        ...
        return interpolate(omega, gamma, time).toFloat()
    }



    companion object {
        fun create(restPositionRuns: Double, overshoot: Double): HarmonicInterpolator {
            val omega = calculateOmega(restPositionRuns)
            val gamma = calculateGamma(restPositionRuns, overshoot, omega)
